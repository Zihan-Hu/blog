...
export type Title = 'unrated' | 'newbie' | 'pupil' | 'specialist' | 'expert' | 'candidate-master' | 'master' | 'international-master' | 'grandmaster' | 'international-grandmaster' | 'legendary-grandmaster' | 'headquarters';
export interface Props {
  title: Title;
}

const RATINGS: Record<Exclude<Title, 'unrated' | 'newbie' | 'headquarters'>, number> = {
  pupil: 1200,
  specialist: 1400,
  expert: 1600,
...
const shownTitle = computed(() => {
  return props.title.split('-').map((v) => capitalize(v)).join(' ');
});

const tip = computed(() => {
  let ans = shownTitle.value;
  switch (props.title) {
    case 'unrated': ans += ', rating = 0'; break;
    case 'newbie': ans += `, rating < ${RATINGS.pupil} and ≠ 0`; break;
    case 'headquarters': break; // noop
    default: ans += `, rating ${RATINGS[props.title]}`;
  }
  return ans;
});
</script>
